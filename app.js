...
client.on("messageCreate", (message) => {
  ...
  if (message.channel.type === 0) {
      ...
      if (command === "verify") {
        ...
        if (!email)
          return message.channel.send(
            "Mohon inputkan email, contoh `fosverify jon.doe@mail.com`"
          );
        ...
        client.users.fetch(message.author.id).then((user) => {
          user.send(
            "Masukkan kode dari email anda dengan command `foskode <kode>`"
          );
        });
        sendMail(email, code);
        message.delete();
    ...
    if (command === "kode") {
      ...
      if (!code)
        return message.channel.send(
          "Mohon inputkan kode, contoh `foskode 123456`"
        );
